...
1. [Switch](tour/flowcontrol/09-switch.html) — Go's switch; only runs the matching case (no fallthrough by default)
2. [Switch Evaluation Order](tour/flowcontrol/10-switch-evaluation-order.html) — Cases are evaluated top to bottom
3. [Switch with No Condition](tour/flowcontrol/11-switch-with-no-condition.html) — `switch {}` is a clean way to write if/else chains
